...
export function UpcomingMezmursPage() {
  ...
  const isPhoneLandscapePresentation = immersiveTouchPresentation && isLandscape
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className={`flex flex-wrap items-start justify-between ${isPhoneLandscapePresentation ? 'gap-2' : 'gap-3'}`}>
            <div>
              <p
                className={`text-xs font-semibold uppercase tracking-[0.18em] ${
                  immersiveTouchPresentation
                    ? isPhoneLandscapePresentation
                      ? 'text-slate-400'
                      : 'text-slate-300'
                    : 'text-brand-700'
                }`}
                ...
                className={`mt-2 font-bold ${
                  immersiveTouchPresentation
                    ? isPhoneLandscapePresentation
                      ...
                      : 'text-2xl text-slate-50 sm:text-3xl'
                    : 'text-3xl text-brand-900 sm:text-5xl'
                }`}
              >
                {isPhoneLandscapePresentation ? (isAm ? 'የመዝሙር ፕሬዘንቴሽን' : 'Mezmur Presentation') : 'Upcoming Mezmurs'}
              </h1>
              <p
                className={`mt-1 ${isPhoneLandscapePresentation ? 'text-xs' : 'text-sm sm:text-base'} ${
                  immersiveTouchPresentation ? 'text-slate-300' : 'text-brand-700 sm:text-xl'
                }`}
              >
                {preview
            ...
            <div className="flex w-full flex-wrap items-center gap-1.5 print:hidden sm:w-auto sm:flex-nowrap sm:gap-2">
              {immersiveTouchPresentation ? (
                <>
                  <button
                    type="button"
                    ...
                    className="inline-flex min-h-10 items-center justify-center rounded-xl border border-slate-500/60 bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-50 enabled:hover:bg-slate-700 enabled:active:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {isAm ? 'ቀዳሚ' : 'Previous'}
                  </button>
                  <button
                    ...
                    className="inline-flex min-h-10 items-center justify-center rounded-xl bg-accent-500 px-3 py-1.5 text-xs font-bold text-slate-950 enabled:hover:bg-accent-400 enabled:active:bg-accent-600 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {isAm ? 'ቀጣይ መዝሙር' : 'Next Mezmur'}
                  </button>
                  <p className="inline-flex min-h-10 items-center rounded-xl border border-slate-700 bg-slate-900 px-2.5 text-[0.7rem] font-semibold text-slate-300">
                    {activeSlide + 1} / {mezmurs.length}
                  </p>
                </>
              ) : null}
              <button
                ...
                className={`inline-flex items-center justify-center rounded-2xl bg-accent-600 font-bold text-white shadow-md shadow-accent-700/30 transition hover:-translate-y-0.5 hover:opacity-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 focus-visible:ring-offset-2 ${
                  immersiveTouchPresentation ? 'min-h-10 px-3 py-1.5 text-xs' : 'min-h-14 px-6 py-3 text-base'
                }`}
              >
              ...
              {isPresentationMode && (!isCoarsePointer || supportsElementFullscreen) ? (
                ...
                className={`inline-flex items-center justify-center rounded-2xl border px-4 py-2 text-sm font-semibold ${
                    immersiveTouchPresentation
                      ? 'min-h-10 border-slate-500/60 bg-slate-800 px-3 py-1.5 text-xs text-slate-50 hover:bg-slate-700'
          ...
          >
            {!isPhoneLandscapePresentation ? (
              <p className={`text-xs font-semibold uppercase tracking-[0.2em] ${immersiveTouchPresentation ? 'text-slate-300' : 'text-brand-700'}`}>
                Mezmur {activeSlide + 1} of {mezmurs.length}
              </p>
            ) : null}
            <h2
              className={`${isPhoneLandscapePresentation ? 'mt-1' : 'mt-3'} font-bold leading-tight ${
                isPresentationMode
                  ? immersiveTouchPresentation
                    ? isPhoneLandscapePresentation
                      ? 'text-xl text-slate-100 sm:text-2xl'
              ...
              <p
                className={`whitespace-pre-wrap font-medium ${
                  isPresentationMode
                    ? immersiveTouchPresentation
                      ? isPhoneLandscapePresentation
                        ? 'text-pretty text-[2.25rem] leading-[1.6] text-slate-50 sm:text-5xl'
            ...
            <div
              className={`mt-4 flex flex-wrap gap-2 print:hidden ${
                immersiveTouchPresentation
                  ? isPhoneLandscapePresentation
                    ? 'border-t border-slate-700/80 pt-2'
                    : 'border-t border-slate-700/80 pt-3'
                  : 'fullscreen:mt-6'
              }`}
            >
            ...
            {isPhoneLandscapePresentation ? null : (
              <div
